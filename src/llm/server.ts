...
const SESSION_TIMEOUT_MS = 2 * 60 * 60 * 1000  // 2 小时
...
async function handleOOC(chatId: string, session: Session, instruction: string, userText: string, senderId: string): Promise<void> {
  ...
  try {
    ...
    // 去掉 LLM 回复里的 [M/D HH:MM] 时间戳前缀，显示和 TTS 都用干净文本
    const strippedReply2 = rawReply.replace(/^(\[\d{1,2}\/\d{1,2}\s+\d{2}:\d{2}\]\s*)+/g, '').trim()
    const { cleanText, styles } = extractVoiceStyle(truncateBrackets(strippedReply2 || rawReply))
    const voiceStyle = styles.length > 0 ? resolveVoiceStyle(styles[0]) : ''
    ...
    // 先 Done./回复，再发语音
    const replyToSend = strippedReply2 || rawReply
    if (progressMsgId) {
